...
def run_renderer(deck: Path, output: Path, cwd: Path, *, language: str = "en") -> subprocess.CompletedProcess:
    """Run the renderer's documented CLI against one deck fixture."""

    environment = os.environ.copy()
    environment["CLAUDE_PLUGIN_ROOT"] = str(ROOT)
    command = [sys.executable, str(RENDERER), str(deck), "-o", str(output), "--language", language]
